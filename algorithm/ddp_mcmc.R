...
#----------------------------- Overall equation -------------------------
mvn_ddp <- function(Y, x, niter, J, burn_in = 1000, thinning = 5, 
  ...
  Xi_C_new <- Xi_C_update(Q_J = Q_J_new, C = C, x = x, x_star_J = x_star_J_new,
                          sigma_star_2_J = sigma_star_2_J_new)
  
  U_C_J_new <- U_C_J_update(Xi_C = Xi_C_new, Q_J = Q_J_new, C = C, x = x,
                            x_star_J = x_star_J_new, sigma_star_2_J = sigma_star_2_J_new, rbf = rbf)
  ...
  for (j in 1:J) {
    ind <- c(1:C)[-log(U_C_J_new[,j])<Xi_C_new*Q_J_new[j]]
    if(length(ind)!=0) {
      uppers <- -(x-x_star_J_new[j])^2/2/(log(-log(U_C_J_new[,j]))-log(Xi_C_new)-log(Q_J_new[j]))
  ...
  for(iter in 2:(niter+1)){
    ...
    Q_J_new <- Q_J_update(Z = Z_new, alpha_0 = alpha_0_new, Xi_C = Xi_C_new,
                          x = x, x_star_J = x_star_J_new, sigma_star_2_J = sigma_star_2_J_new)
    ...
    # 4-2) ------- x_star_J -------
    x_star_J_new <- x_star_J_update(r_J = r_J_new, s_2 = s_2_new, Z = Z_new, x = x, C = C,
                                    sigma_star_2_J = sigma_star_2_J_new, U_C_J = U_C_J_new,
                                    Xi_C = Xi_C_new, Q_J = Q_J_new)
    ...
    P_C_J_new <- t(sapply(x,function(xx) {
      # log-scale
      LP <- log(Q_J_new)-(xx-x_star_J_new)^2/2/sigma_star_2_J_new
      nc <- -max(LP)
