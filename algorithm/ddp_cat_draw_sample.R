##---------------- MCMC Simulation --------------
rtgamma <- function(lower=NULL, upper=NULL, shape, rate, init, burn_in=0){
  
  for (i in 1:(burn_in+1)) {
    B_v <- 0
    if(shape!=1){
      B_v <- exp(log(runif(1))*(1/(shape-1))+log(init))
    }
    
    # no upper bound
    if((!is.null(lower)) & (is.null(upper))){
      B_v <- max(c(lower,B_v))
      val <- rexp(1, rate=rate)+B_v
    }else {
      B_v <- max(c(lower,B_v))
      I <- 1-exp(-rate*(upper-B_v))
      
      if(I==0){
        warning("Approximating truncated gamma due to precision issues")
        val <- B_v+(upper-B_v)*runif(1)
      }else{
        val <- B_v-(1/rate)*log(1-runif(1)*I)
      }
    }
...
rtdirichlet <- function(alpha, lower=NULL, upper=NULL, init, burn_in=1000){
  
  # dimension
  G <- length(alpha)
  # turn into a Gamma
  Z_gamma <- init*rgamma(1,shape=sum(alpha),rate=1)
  
  for (i in 1:(burn_in+1)) {
    for(g in 1:G){
      Z_rest <- Z_gamma[-g]
      rest_ind <- setdiff(1:G, g)
      
      # find bounds for Z_gamma[g]
      lower_z <- max((1-upper[rest_ind])/upper[rest_ind]*Z_rest -
                       (rep(1,G-1)-diag(G-1)) %*% Z_rest)
      lower_z <- max(c(lower_z, lower[g]/(1-lower[g])*sum(Z_rest)))
      
      if(max(lower[rest_ind]) == 0 && upper[g] == 1){ #no upper bound for Z_gamma[g]
        if(lower_z==0){
          Z_gamma[g] <- rgamma(1, alpha[g], 1)
        }else{
          Z_gamma[g] <- rtgamma(lower=lower_z,shape=alpha[g],rate=1,init = Z_gamma[g])
          # truncdist::rtrunc(1,'gamma',a=lower_z,b=Inf,shape=alpha[g],scale=1)
        }
      }else{
        upper_z <- min((1-lower[rest_ind])/lower[rest_ind]*Z_rest -
                         (rep(1,G-1)-diag(G-1)) %*% Z_rest)
        upper_z <- min(c(upper_z, upper[g]/(1-upper[g])*sum(Z_rest)))
        Z_gamma[g] <- rtgamma(lower=lower_z,upper=upper_z,shape=alpha[g],rate=1,init = Z_gamma[g])
        # truncdist::rtrunc(1,'gamma',a=lower_z,b=upper_z,shape=alpha[g],scale=1)
...
Xi_C_update <- function(Q_J, C, x, xc, x_star_J, sigma_star_2_J, rho){
  J <- length(Q_J)
  ...
  rates <- unlist(lapply(1:C, function(c) {
    ...
    log_cat_value <- log(rho[,xc[c]])
    log_temp <- log(Q_J)+log_rbf_value+log_cat_value
    log_K <- max(log_temp)
...
Q_J_update <- function(Z, alpha_0, Xi_C, x, xc, x_star_J, sigma_star_2_J, rho){
  J <- length(x_star_J)
  ...
  shape_rate_param <- do.call(cbind,lapply(1:J, function(j) {
    ...
    log_cat_value <- log(rho[j,xc])
    log_temp <- log(Xi_C)+log_rbf_value+log_cat_value
    log_K <- max(log_temp)
    rate <- 1+exp(log_K)*sum(exp(log_temp-log_K))
    # sum(Xi_C*rbf(t, x_star_J[j], sigma_star_2_J[j]))
...
allocation_variables_update <- function(Y, x, xc, mu_star_1_J, Sigma_star_1_J, Q_J, 
                                        x_star_J, sigma_star_2_J, rho){
  ...
  # n * J
  LP1 <- -(x_mat-x_star_mat)^2/2/sigma_star_2_mat+log(Q_mat)+log(rho_mat)
...
U_C_J_update <- function(Xi_C, Q_J, C, x, xc, x_star_J, sigma_star_2_J, rho, rbf){
  J <- length(Q_J)
  
  U_C_J <- do.call(cbind, lapply(1:J, function(j) {
    K_C_J_D <- exp(-Xi_C*Q_J[j]*rbf(x,x_star_J[j],sigma_star_2_J[j])*rho[j,xc])
...
##----------- Simulation of kernel parameters rho_J_R in categorical kernel -------------
rho_J_R_update <- function(rho, x, xc, Z, gamma, U_C_J, Xi_C, Q_J,x_star_J, sigma_star_2_J, 
                           rbf, trunc_dirichlet_burn){
  ...
  rho_new <- lapply(1:J, function(j) {
    
    upper <- c()
    # full conditional is a (truncated) dirichlet
    concentrate_param <- c()
    
    # upper bound for each rho[j,r]
    for (r in 1:R) {
      ind <- (xc==r)
      temp <- -log(U_C_J[,j])/Xi_C/Q_J[j]/rbf(x,x_star_J[j],sigma_star_2_J[j])
      upper[r] <- min(c(1, temp[ind]))
      
      # update corresponding concentration param
      n <- sum((Z==j) & (xc==r))
      concentrate_param[r] <- n+gamma[r]
    }
    
    if(all(upper==1)){
      val <- rdirichlet(1,concentrate_param)
    }else{
      # truncated dirichlet
      val <- rtdirichlet(alpha=concentrate_param, lower=rep(0,R), upper=upper, 
                         init = rho_old[j,], burn_in = trunc_dirichlet_burn)
    }
    
  })
  
  rho_new <- do.call(rbind, rho_new)
...
x_star_J_update <- function(r_J, s_2, Z, x, xc, C, sigma_star_2_J, U_C_J, Xi_C, Q_J, rho){
  J <- length(Q_J)
  ...
  for (j in 1:J){
    ...
    ind <- c(1:C)[-log(U_C_J[,j])<Xi_C*Q_J[j]*rho[j,xc]]
    if(length(ind)==0) {
      truncate <- FALSE
    }else{
      ...
      i_all_complement <- lapply(ind, function(c) {
        temp <- -2*sigma_star_2_J[j]*(log(-log(U_C_J[c,j]))-log(Xi_C[c])-log(Q_J[j])-log(rho[j,xc[c]]))
...
sigma_star_2_J_update <- function(sigma_star_2_J, h_J, m_2, Z, x, xc, C, x_star_J, U_C_J, Xi_C, Q_J, rho,
  ...
  for (j in 1:J) {
    ...
    if(length(ind)==0) {
      ...
      truncate <- TRUE
      uppers <- -(x-x_star_J[j])^2/2/(log(-log(U_C_J[,j]))-log(Xi_C)-log(Q_J[j])-log(rho[j,xc]))
